...
func TestReportTypeNormalize(t *testing.T) {
	tests := map[string]struct {
		...
		"empty stays empty":      {in: "", expected: ReportTypeNil},
		"unknown value upper":    {in: "yaml", expected: "YAML"},
	}
...
func TestReportTypeValid(t *testing.T) {
	tests := map[string]struct {
		...
		"empty is valid (no report)": {in: ReportTypeNil, expected: true},
		...
		"unknown is invalid":         {in: "YAML", expected: false},
	}

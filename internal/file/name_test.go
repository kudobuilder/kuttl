...
func TestParse(t *testing.T) {
	tests := []Info{
		...
		{
			Type:       TypeAssert,
			FullName:   "00-assert.gotmpl.yaml",
			BaseName:   "00-assert.gotmpl.yaml",
			IsTemplate: true,
			HasIndex:   true,
			Index:      0,
			StepName:   "",
		...
		{
			Type:       TypeError,
			FullName:   "00-errors.gotmpl.yaml",
			BaseName:   "00-errors.gotmpl.yaml",
			IsTemplate: true,
			HasIndex:   true,
			Index:      0,
			StepName:   "",
		...
		{
			Type:       TypeApply,
			FullName:   "00-foo.gotmpl.yaml",
			BaseName:   "00-foo.gotmpl.yaml",
			IsTemplate: true,
			HasIndex:   true,
			Index:      0,
			StepName:   "foo",
		...
		{
			Type:     TypeUnknown,
			FullName: "1-foo.gotmpl",
			BaseName: "1-foo.gotmpl",
			Error:    fmt.Errorf("name does not follow pattern %q", fileNamePattern),
		},
		{
			Type:     TypeUnknown,

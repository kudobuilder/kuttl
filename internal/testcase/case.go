...
//     comes to file. At this point a candidate namespace name is determined.
...
//     For ephemeral namespaces, if the chosen name collides with a pre-existing namespace,
//     a new name is generated and the creation is retried.
//     4b. if the namespace name changed due to a collision, reloads test steps so that
//     templates re-expand with the correct namespace.
//     4c. for each step: sets the step up, prepares its client if lazy-loaded, and runs the step
type Case struct {
	steps              []*step.Step
	name               string
	dir                string
...
// createNamespace creates (or reuses) the test namespace on the given cluster.
// If retryOnCollision is true and the namespace is ephemeral (not user-supplied),
// a collision with a pre-existing namespace (e.g. left over from a crashed run or
// generated by another test case in this run) will cause a new name to be generated
// and retried. The returned nsRegenerated flag indicates whether the namespace name
// was changed during this call.
func (c *Case) createNamespace(test T, cl clientWithKubeConfig, retryOnCollision bool) (nsRegenerated bool, err error) {
	ctx := test.Context()
	if c.timeout > 0 {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, time.Duration(c.timeout)*time.Second)
	...
	for {
		...
		if k8serrors.IsAlreadyExists(err) && retryOnCollision && !c.ns.userSupplied {
			// Ephemeral namespace collision: generate a new name and retry.
			old := c.ns.name
			c.ns.name = fmt.Sprintf("kuttl-test-%s", petname.Generate(2, "-"))
			c.templateEnv.Namespace = c.ns.name
			nsRegenerated = true
			cl.Logf("Namespace %q already exists, retrying with %q", old, c.ns.name)
			continue
		}

		// For user-supplied namespaces or non-collision errors, fall through to didNsExist.
		var nsExisted bool
		nsExisted, err = c.didNsExist(ctx, err, cl)
		if err != nil {
			return nsRegenerated, fmt.Errorf("failed to create test namespace %q: %w", c.ns.name, err)
		}
		...
		return nsRegenerated, nil
	}

	// Namespace was freshly created (no collision or after successful retry).
	if !c.skipDelete {
		test.Cleanup(func() {
			if err := c.deleteNamespace(cl); err != nil {
				test.Error(err)
...
func (c *Case) Run(test *testing.T, rep report.TestReporter) {
	...
	nsRegenerated, err := c.setup(test)
	if err != nil {
		setupReport.Failure(err.Error())
		test.Fatal(err)
	}

	// If the namespace name changed due to a collision during setup, reload
	// test steps so that templates re-expand with the correct namespace.
	if nsRegenerated {
		if err := c.LoadTestSteps(); err != nil {
			setupReport.Failure(err.Error())
			test.Fatal(err)
		}
	}
	...
	for _, testStep := range c.steps {
		...
		// retryOnCollision is false here: for lazy-loaded kubeconfigs (multi-cluster),
		// the same namespace name must be used across all clusters.
		if testStep.KubeconfigLoading == v1beta1.KubeconfigLoadingLazy {
			...
			} else if _, err = c.createNamespace(test, clientWithKubeConfig{cl, testStep.Kubeconfig, c.logger}, false); err != nil {
...
func (c *Case) setup(test *testing.T) (nsRegenerated bool, err error) {
	...
	for _, cl := range clients {
		regenerated, err := c.createNamespace(test, cl, true)
		if err != nil {
			return nsRegenerated, err
		}
		nsRegenerated = nsRegenerated || regenerated
	}
	return nsRegenerated, nil
}
